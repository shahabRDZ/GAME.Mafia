"""Lab room WebSocket handlers, game engine, and bot AI."""
import random
import threading
...
from models import User, LabRoom, LabPlayer, LabMessage, BotMemory
from services.state import (
    sid_to_user, user_to_sid,
    lab_votes, lab_revotes, lab_night_actions,
    bot_game_memory, bot_used_messages,
)
...
BOT_PERSONALITIES = {
    ...
    "امیر": {"type": "aggressive", "label": "پرانرژی و تهاجمی"},
...
def get_lab_room_data(room):
    """Serialize lab room data for client."""
    players = []
...
def schedule_turn_timer(code, current_slot, day_number):
    """Schedule auto-advance for day_talk after 40 seconds."""
    app = _get_app()
    def advance():
...
def schedule_vote_advance(code, current_slot, day_number):
    """Advance sequential voting after 5 seconds."""
    app = _get_app()
    def advance():
...
def schedule_revote_advance(code, current_slot, day_number):
    """Advance sequential revote after 5 seconds."""
    app = _get_app()
    def advance():
        _time_module.sleep(6)
        with app.app_context():
...
def schedule_bazpors_vote_advance(code, current_slot, day_number):
    """Auto-advance bazpors vote after 5s."""
    app = _get_app()
    def advance():
        _time_module.sleep(6)
        with app.app_context():
...
def start_sequential_voting(code):
    """Start voting: for each alive player, everyone votes yes/no."""
    room = LabRoom.query.filter_by(code=code).first()
    if not room:
        return
...
def resolve_voting(code):
    ...
    room_key = f"{code}_{room.day_number}"
    vote_counts = lab_votes.get(room_key, {})
...
def generate_bot_defense(code, bot_player):
    ...
    def send():
        ...
        with app.app_context():
            ...
            if not room:
                return
            # Check for both regular defense and bazpors defense phases
            ...
            elif "bazpors_defense" in room.phase:
                if room.defense_player_id != bot_player.id:
                    return
    ...
    def vote():
        ...
        with app.app_context():
            ...
            if bot_player.team == "mafia" and defense_player.team == "mafia":
                ...
                decision = random.choice(["eliminate", "eliminate", "keep"])
...
def get_fallback_bot_message(role_name, team, day_number):
    """Fallback messages when no learned memory exists."""
    if team == "mafia":
        msgs = [
            "من بی\u200cگناهم، بهم اعتماد کنید",
            "اشتباه نکنید، من شهروندم",
            "من دارم کمکتون میکنم",
            "بذارین من هم نظرمو بگم",
        ]
    else:
        msgs = [
            "بیاین منطقی فکر کنیم",
            "من مطمئنم یکی اینجا مشکوکه",
            "باید دقت کنیم",
            "رفتارها رو ببینید",
        ]
        ...
        with app.app_context():
            for p in room.players:
                ...
                if name and name in content:
                    reaction = random.choice(["like", "like", "dislike"])
                    socketio.emit("lab_reaction", {
                        "message_id": message_id,
                        "reaction": reaction,
                        "from_user": name
...
def register_lab_handlers(socketio, app):
    """Register all lab room socket events."""
    global _flask_app
    _flask_app = app
